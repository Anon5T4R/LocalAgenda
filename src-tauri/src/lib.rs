        ...
        .setup(|app| {
            ...
            if let Some(win) = app.get_webview_window("main") {
                ...
                win.on_window_event(move |event| {
                    ...
                    // (min/max/fechar) mortos até um resize — acontece no
                    // startup e a cada hide→show da bandeja (tauri#13440,
                    // tauri#11856). O toggle de `resizable` em cada foco força
                    // o GTK a revalidar as decorações. Remover quando o tauri
                    // puxar o tao 0.36 (via wry 0.56).
                    #[cfg(target_os = "linux")]
